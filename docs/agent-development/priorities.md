...
canonical loop is documented in [agent-workflow.md](workflow.md).
...
[headless-state-fixtures.md](headless-fixtures.md):
...
| Equality-policy benchmarks and tuning                      | **P1**                                                 | **M**                     | **Low**           | **No**          | The safe shallow default is established; measure wide result shapes and keep identity, domain, or deep overrides explicit and inspectable.              |
...
- Introduce any improved `state` vocabulary additively, migrate generated
  templates first, and retain the compatibility path required by the production
  contract. Defer an unavoidable removal to a future major release.

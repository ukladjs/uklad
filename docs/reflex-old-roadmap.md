...
> **Superseded (July 2026)** by the phased [Reflex roadmap](../ROADMAP.md). Kept for reference: the **agent indexing model** and the **Redux/Zustand feature-parity table** below remain valid, and open items here have been folded into the new roadmap's phases.

Prioritized improvements for [Reflex](../packages/reflex), ordered by impact. DevTools-specific work is tracked in the [DevTools roadmap](devtools-roadmap.md). Items marked **(pairs with …)** need coordinated changes across both packages.

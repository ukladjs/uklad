...
const disposeEvents = todoRuntime.registerModule((registrar) => {
  ...
  registrar.regEvent(EVENT_IDS.TOGGLE_DONE, ({ draftState }, id) => {
  ...
  registrar.regEvent(EVENT_IDS.DELETE_TODO, ({ draftState }, id) => {
    draftState.todos.delete(id);
  });

  registrar.regEvent(EVENT_IDS.SAVE, ({ draftState }, id, newTitle) => {
    const todo = draftState.todos.get(id);
    if (todo) {
  ...
  registrar.regEvent(EVENT_IDS.SET_SHOWING, ({ draftState }, showing) => {

...
The plugin ships the Reflex skill (workflow, conventions, progressive references) and the [DevTools](https://github.com/flexsurfer/reflex-devtools) MCP configuration. From there the agent handles the project itself: installs dependencies, wires dev-only tracing, starts the project-local devtools server, and **verifies its own changes at runtime** instead of re-reading source files — `dispatch_event` returns the state patches an event committed, the effects it emitted, or the error if it failed.

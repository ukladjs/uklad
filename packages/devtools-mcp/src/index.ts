...
1. app_status — discover runtimes and select one. It lists stable runtimeId values and reports whether the selected app is connected, browser/React Native/headless, current committed/published state revisions, tracing state, handler counts, and sessionEpoch. Call it first after a cold start and after any reload; a changed sessionEpoch means the DevTools connection session changed and server-stored trace ids were invalidated. A transient reconnect can leave the runtime state intact.
...
5. dispatch_and_wait — preferred act-and-verify path for operation-enabled runtimes. It returns the DevTools-owned operation snapshot after the joined event cascade settles: identity/status, event lineage, committed/published revisions, pending work, and errors. Each operation eventInstanceId is also present on related trace rows; pass it to get_traces when trace evidence is useful. Use dispatch_event only with older runtimes that lack the operation capability.
6. get_traces — compact rows of recent activity, including what you did not initiate (user clicks, timers, subscriptions). Event rows include state disposition/revision facts that match dispatch_and_wait events. Pass eventInstanceId from dispatch_and_wait to retrieve exact related rows, or drill into one trace with get_trace, passing the get_traces response's runtimeId and sessionEpoch so a restart fails explicitly; never page through full trace details.

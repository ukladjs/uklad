...
export function assertEvent(event: OperationEventVector): void {
  if (!Array.isArray(event) || event.length === 0 || typeof event[0] !== 'string') {
    throw new Error('[Reflex Devtools] operation event must be a non-empty event vector.');
  }
}
...
export function assertOptions(options: OperationOptions): void {
  ...
  for (const key of Object.keys(options)) {
    if (!allowed.has(key)) throw new Error(`[Reflex Devtools] Unknown operation option '${key}'.`);
  }
  if (options.completion !== undefined && options.completion !== 'cascade-published') {
    throw new Error("[Reflex Devtools] operation completion must be 'cascade-published'.");
  }
  ...
  if (options.expectedRevision !== undefined && (!Number.isSafeInteger(options.expectedRevision) || options.expectedRevision < 0)) {
    throw new Error('[Reflex Devtools] operation expectedRevision must be a non-negative safe integer.');
  }
  if (options.observe && options.observe.length > 64) {
    throw new Error('[Reflex Devtools] operation observe accepts at most 64 subscription queries.');
  }
}

export function validateInput(event: OperationEventVector, options: OperationOptions): void {
  try {
    clone({ event, observations: options.observe ?? [], executionContext: options.executionContext ?? null });
  } catch (error: unknown) {
    throw new Error(
      '[Reflex Devtools] operation input must be structured-cloneable so its evidence is immutable.',
      { cause: error },
    );
  }
...
export function normalizeTimeout(value: number | undefined): number {
  ...
  if (!Number.isFinite(timeout) || timeout < 0 || timeout > MAX_TIMEOUT_MS) {
    throw new Error(`[Reflex Devtools] operation timeoutMs must be between 0 and ${MAX_TIMEOUT_MS}.`);
  }
  return timeout;
}
...
export function fingerprintOperation(event: OperationEventVector, options: OperationOptions): string {
  return JSON.stringify(fingerprintInput(event, options));
}

function fingerprintInput(event: OperationEventVector, options: OperationOptions): unknown {
  return {
    event,
...
function assertJsonInput(value: unknown): void {
  ...
  const visit = (item: unknown): void => {
    ...
    if (typeof item !== 'object') throw new Error('[Reflex Devtools] idempotent operation input must be JSON-compatible.');
    if (seen.has(item)) throw new Error('[Reflex Devtools] idempotent operation input must not contain cycles.');
    seen.add(item);
    if (Array.isArray(item)) {
      ...
      if (prototype !== Object.prototype && prototype !== null) {
        throw new Error('[Reflex Devtools] idempotent operation input must use JSON arrays and plain objects.');
      }
...
function validateIdentifier(value: string, field: string): void {
  if (!IDENTIFIER_PATTERN.test(value)) {
    throw new Error(`[Reflex Devtools] ${field} must be 1-256 characters and contain only letters, numbers, dot, underscore, colon, or hyphen.`);
  }
}

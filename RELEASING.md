...
The coordinated package set lives in [`release.json`](release.json). Keep it aligned with package manifests, peer ranges, the agent toolkit's `versions.json`, and the website release constants.
...
1. Update package versions and tags in `release.json`, then update package manifests and compatible peer ranges.
2. Update `CHANGELOG.md`, package README pins, the agent toolkit compatibility set, and website metadata/install commands.
3. Run `pnpm run release:check`. Inspect every dry-run file list and package size.

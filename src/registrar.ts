...
const kindToIdToHandler = createHandlerRegistry();
...
>(kind: K, id: Id, handlerFn: T): T {
  if (kindToIdToHandler[kind][id]) {
    consoleLog('warn', `[reflex] overwriting ${kind} handler for:`, id);
  }

  (kindToIdToHandler[kind] as unknown as Record<string, RegistryHandler>)[id] = handlerFn;
  return handlerFn;
}

/** Register a framework handler that public clear operations must restore. */
export function registerSystemHandler<
  K extends HandlerKind,
  T extends HandlerByKind[K] = HandlerByKind[K],
>(kind: K, id: Id, handlerFn: T): T {
  (systemHandlers[kind] as unknown as Record<string, RegistryHandler>)[id] = handlerFn;
  (kindToIdToHandler[kind] as unknown as Record<string, RegistryHandler>)[id] = handlerFn;
  return handlerFn;
}
...
export function clearHandlers(kind: HandlerKind, id: string): void;
export function clearHandlers(kind?: HandlerKind, id?: string): void {
  if (kind == null || kind === 'sub' || kind === 'subDeps') assertSubscriptionsCanBeCleared();
  clearHandlerEntries(kind, id);
}

function clearHandlerEntries(kind?: HandlerKind, id?: string): void {
  if (kind == null) {
    resetHandlerRecord('event');
    resetHandlerRecord('fx');
    resetHandlerRecord('cofx');
    resetHandlerRecord('sub');
    resetHandlerRecord('subDeps');
    resetHandlerRecord('error');
    clearRootSubSources();
    ...
    if (kind === 'sub' || kind === 'subDeps') {
      clearSubscriptionDefinitionEntries();
    } else {
    ...
    const handlerExists = hasHandler(kind, id);
    if (kind === 'sub' || kind === 'subDeps') {
      clearSubscriptionDefinitionEntries(id);
    } else {
      restoreOrDeleteHandler(kind, id);
...
function resetHandlerRecord(kind: HandlerKind): void {
  const handlers = createHandlerRecord<RegistryHandler>();
  Object.assign(handlers, systemHandlers[kind]);
  (kindToIdToHandler as unknown as Record<HandlerKind, Partial<Record<string, RegistryHandler>>>)[
    kind
  ] = handlers;
}

function restoreOrDeleteHandler(kind: HandlerKind, id: Id): void {
  const systemHandler = systemHandlers[kind][id];
  if (systemHandler) {
    (kindToIdToHandler[kind] as unknown as Record<string, RegistryHandler>)[id] = systemHandler;
  } else {
    delete kindToIdToHandler[kind][id];
  }
}

function clearSubscriptionDefinitionEntries(id?: Id): void {
  if (id == null) {
    resetHandlerRecord('sub');
    resetHandlerRecord('subDeps');
    clearRootSubSources();
  ...
  delete kindToIdToHandler.sub[id];
  delete kindToIdToHandler.subDeps[id];
  clearRootSubSource(id);
  clearSubscriptionCacheEntriesForId(id);
  clearSubConfigs(id);
}

export function hasHandler(kind: HandlerKind, id: string): boolean {
  return !!kindToIdToHandler[kind][id];
}

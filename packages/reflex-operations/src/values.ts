...
export function copyPatch(patch: RuntimeLifecyclePatch): OperationPatch {
  return {
    op: patch.op,
    path: [...patch.path],
...
export function copyOperationPatch(patch: OperationPatch): OperationPatch {
  return {
    op: patch.op,
    path: [...patch.path],
    ...('value' in patch ? { value: snapshotValue(patch.value) } : {}),
...
export function copyError(error: OperationError): OperationError {
  return { ...error };
}

export function cloneExecutionContext(value: OperationExecutionContext): OperationExecutionContext {
  return {
    profile: value.profile,
    source: value.source,
    enforced: false,
    ...(value.defaultEffectMode ? { defaultEffectMode: value.defaultEffectMode } : {}),
...
function cloneFallback<T>(value: T, seen: WeakMap<object, unknown>): T {
  ...
  if (ArrayBuffer.isView(value)) {
    if (value instanceof DataView) {
      ...
      return new Constructor(cloneFallback(value.buffer, seen), value.byteOffset, value.byteLength) as T;
    }
  ...
  if (value instanceof Map) {
    ...
    seen.set(value, copy);
    for (const [key, entry] of value) copy.set(cloneFallback(key, seen), cloneFallback(entry, seen));
    return copy as T;
  }
  ...
  if (!isPlainObject) throw new TypeError('Value is not structured-cloneable.');
  const copy = Object.create(prototype) as Record<string, unknown>;
  seen.set(value, copy);
  for (const key of Object.keys(value)) copy[key] = cloneFallback((value as Record<string, unknown>)[key], seen);
  return copy as T;
}

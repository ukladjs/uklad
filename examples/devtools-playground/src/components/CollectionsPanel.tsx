...
function CollectionsPanel() {
  ...
  const addPermission = () => {
    ...
    runtime.dispatch(['add-permission', options[Math.floor(Math.random() * options.length)]]);
  };
  ...
  const createComplexStructure = () => runtime.dispatch(['create-complex-map-set-structure']);
    ...
    <div>
      <section className="maps-sets-section">
        ...
        <p className="sub-info">
          Subscriptions: <code>userMap</code>, <code>permissionsSet</code>,{' '}
          <code>nestedCollections-comp</code>
        </p>

        <div className="action-buttons" style={{ marginBottom: '1.5rem' }}>
          <button onClick={addUserToMap} className="map-button">
            Add User to Map
          </button>
          <button
            onClick={() =>
              runtime.dispatch(['update-user-in-map', 'user-1', { role: 'super-admin' }])
            }
          ...
          <button
            onClick={() => runtime.dispatch(['remove-user-from-map', 'user-2'])}
            className="map-button"
          >
            Remove user-2
          </button>
          ...
          <button
            onClick={() => runtime.dispatch(['remove-permission', 'read'])}
            className="set-button"
          >
            Remove "read"
          </button>
          <button onClick={toggleUserRole} className="set-button">
            Toggle Alice's Role
          </button>
          <button onClick={createComplexStructure} className="complex-button">
        ...
        <div className="nested-collections-display">
          <div className="collection-item">
            ...
            {userMap &&
              Array.from(userMap.entries()).map(([key, val]) => (
          ...
          <div className="collection-item">
            ...
            {nestedCollections?.rolesMap &&
              (Array.from(nestedCollections.rolesMap.entries()) as [string, Set<string>][]).map(
                ([role, perms]) => (
          ...
          <div className="collection-item">
            ...
            {nestedCollections?.userPermissions &&
              (
                Array.from(nestedCollections.userPermissions.entries()) as [string, Set<string>][]
              ).map(([user, perms]) => (

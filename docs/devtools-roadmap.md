...
- [ ] **Shape mode for `get_app_state`.**
  Add `depth` or `shape: true` returning keys + types + collection sizes — the runtime equivalent of reading `state.ts`, and the right first call on an unfamiliar large app. (`get_app_state` already takes a `path` for scoped reads; shape mode is the discovery step that tells the agent which paths are worth reading — the full dump is unusable on a real-sized state.)
